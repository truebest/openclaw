...
describe("handleFluxerInbound", () => {
  ...
  it("keeps messages addressed to another mentioned user passive even if the model asks to send", async () => {
    const runtime = createRuntime();
    setFluxerRuntime(runtime);
    const cfg = {
      messages: {
    ...
    await dispatchReply.mock.calls[0]?.[0].delivery.deliver({ text: "wrong bot reply" });

    expect(sendFluxerTextMock).not.toHaveBeenCalled();
  });

  it("keeps mentioned room events passive when the current bot id is unavailable", async () => {
    const runtime = createRuntime();
    setFluxerRuntime(runtime);
    const cfg = {
      messages: {
    ...
    await handleFluxerInbound({
      account: createAccount({ botUserId: undefined }),
      config: cfg,
      message: createMessage({
        content: "<@bot-2> как дела?",
        mentions: [{ id: "bot-2" }],
    ...
    await dispatchReply.mock.calls[0]?.[0].delivery.deliver({ text: "unknown bot reply" });
